...
void main() {
  group('WordCatalog', () {
    test('TR locale için TR kelimeleri döner', () {
      final words = WordCatalog.wordsForLocale('tr');
    ...
    test('desteklenmeyen locale EN fallback döner', () {
      final words = WordCatalog.wordsForLocale('de');
      expect(words, isNotEmpty);
      expect(words.any((w) => w.id.startsWith('en_')), isTrue);
    });

    test('her locale için 50 kelime vardır', () {
      expect(WordCatalog.wordCountForLocale('en'), 50);
      expect(WordCatalog.wordCountForLocale('tr'), 50);
    });

    test('kelimeler uzunluğa göre sıralıdır', () {
      final words = WordCatalog.wordsForLocale('en');
