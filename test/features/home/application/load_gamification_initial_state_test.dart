...
void main() {
  ...
  group('LoadGamificationInitialState', () {
    ...
    test('kalıcılık değerlerini GamificationInitialState olarak birleştirir', () async {
      ...
      when(
        () => mockPersistence.getStringList(GamificationConstants.keyUnlockedBadges),
      ...
      final state = result.fold((_) => throw StateError('Right beklenir'), (s) => s);
    ...
    test('null sayısal alanlarda varsayılan sıfır ve boş listeler kullanılır', () async {
      when(() => mockPersistence.getInt(any())).thenAnswer((_) async => right(null));
      when(() => mockPersistence.getStringList(any())).thenAnswer((_) async => right(null));
      when(() => mockPersistence.getString(any())).thenAnswer((_) async => right(null));
