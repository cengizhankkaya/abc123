...
import 'package:abc123/features/home/presentation/avatar/fluttermoji_functions.dart';
...
class FluttermojiController extends ChangeNotifier {
  ...
  /// Kullanıcının her özellik için seçtiği indeksi saklar.
  /// Örn: selectedOptions["eyes"] → seçilen göz tipinin indeksi
  Map<String?, dynamic> selectedOptions = <String?, dynamic>{};

  Future<void> _init() async {
    final tempOptions = await getFluttermojiOptions();
    selectedOptions = tempOptions;
    _fluttermoji = getFluttermojiFromOptions();
    notifyListeners();
  }

  /// Önizlemeyi günceller (kaydetmez).
...
<use fill="#D0C6AC" xlink:href="#path-5"></use>$skinSvg<path d="M156,79 L156,102 C156,132.927946 130.927946,158 100,158 C69.072054,158 44,132.927946 44,102 L44,79 L44,94 C44,124.927946 69.072054,150 100,150 C130.927946,150 156,124.927946 156,94 L156,79 Z" id="Neck-Shadow" opacity="0.100000001" fill="#000000" mask="url(#mask-6)"></path></g>$clothe<g id="Face" transform="translate(76.000000, 82.000000)" fill="#000000">$mouthSvg$facialHair$noseSvg$eyesSvg$eyebrowsSvg$accessory</g>$hair</g></g></g></g></svg>''';
  ...
  String getComponentSVG(String? attributeKey, int? attributeValueIndex) {
    switch (attributeKey) {
      ...
      case 'eyeType':
        return '''<svg width="20px" height="20px" viewBox="-3 -30 120 120">${eyes[EyeType[attributeValueIndex!]] as String}</svg>''';

      case 'eyebrowType':
        return '''<svg width="20px" height="20px" viewBox="-3 -50 120 120">${eyebrow[EyebrowType[attributeValueIndex!]] as String}</svg>''';

      case 'mouthType':
        return '''<svg width="20px" height="20px" viewBox="0 10 120 120">${mouth[MouthType[attributeValueIndex!]] as String}</svg>''';

      case 'accessoriesType':
        if (attributeValueIndex == 0) return emptySVGIcon;
        return '''<svg width="20px" height="20px" viewBox="-3 -50 120 170" >${accessories[AccessoriesType[attributeValueIndex!]] as String}</svg>''';
...
\t\t\t\t\t\t<use fill="#D0C6AC" xlink:href="#path-5"></use>
        ${skin[SkinColor[attributeValueIndex!]] as String}\t<path d="M156,79 L156,102 C156,132.927946 130.927946,158 100,158 C69.072054,158 44,132.927946 44,102 L44,79 L44,94 C44,124.927946 69.072054,150 100,150 C130.927946,150 156,124.927946 156,94 L156,79 Z" id="Neck-Shadow" opacity="0.100000001" fill="#000000" mask="url(#mask-6)"></path>
\t\t\t\t</g>
\t\t</g>
\t</g>
</svg>''';
