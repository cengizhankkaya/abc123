      ...
      execute(() async {
        ...
        for (final module in progressList) {
          if (module.moduleName == 'numbers') {
            if (module.strugglingItems.isNotEmpty ||
                (module.accuracyRate < 75.0 && module.completionPercentage > 0)) {
              final itemsText = module.strugglingItems.isNotEmpty
                  ? module.strugglingItems.take(3).join(', ')
                  : (isTurkish ? 'bazı rakamlar' : 'some digits');
              recommendations.add(
                Recommendation(
                  title: isTurkish ? 'Rakam Pratiğini Güçlendirin' : 'Strengthen Number Practice',
            ...
            if (module.strugglingItems.isNotEmpty ||
              ...
              recommendations.add(
                Recommendation(
                  title: isTurkish ? 'Harf Çizimlerine Odaklanın' : 'Focus on Letter Drawings',
                  description: isTurkish
            ...
            if (module.strugglingItems.isNotEmpty ||
                (module.accuracyRate < 70.0 && module.completionPercentage > 0)) {
              recommendations.add(
                Recommendation(
                  title: isTurkish ? 'Geometrik Şekilleri Tekrar Edin' : 'Review Geometric Shapes',
                  description: isTurkish
            ...
            if (module.strugglingItems.isNotEmpty ||
              ...
              recommendations.add(
                Recommendation(
                  title: isTurkish ? 'Matematik İşlemlerini Geliştirin' : 'Improve Math Operations',
                  description: isTurkish
            ...
            if (module.completionPercentage < 20.0 &&
                progressList
                    .any((p) => p.moduleName == 'letters' && p.completionPercentage > 30.0)) {
              recommendations.add(
                Recommendation(
                  title: isTurkish ? 'Kelime Çizmeye Başlama Zamanı' : 'Time to Build Words',
                  description: isTurkish
