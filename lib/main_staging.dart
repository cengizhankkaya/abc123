...
/// Staging (test) ortamı giriş noktası (`01_project_structure.md` — Build Flavors).
///
/// Çalıştırmak için:
/// ```sh
/// flutter run --dart-define=APP_ENV=staging -t lib/main_staging.dart
/// ```
void main() async {
  await bootstrap();
  assert(
    AppEnvironment.current == AppEnvironment.staging,
    'main_staging.dart yalnızca APP_ENV=staging ile çalıştırılmalıdır.',
  );
